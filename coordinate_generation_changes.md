...
- 在 get_wall_placement_info 中，墙体底边终点索引从 1 改为 3。
- 影响范围：绘画、照片、电视等所有调用该函数的墙挂物体。

作用:

- 与改动 2（窗户定位）一致，统一使用新 polygon 顺序的正确底边端点。
- 修复墙挂物体（如 Television_14、Wall_Decor_Photo_7）沿墙方向定位偏移问题。
...
5. 墙挂物体（绘画、电视等）是否位于正确墙段，沿墙方向无偏移。
